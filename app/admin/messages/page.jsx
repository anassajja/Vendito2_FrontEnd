...
const User_admin = () => {
    ...
    const { logOut } = useContext(AuthContext); // Destructure the logOut function from the AuthContext
    // Initialize messages to an empty array if it's undefined
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        fetchData(); // Fetch the messages
      }, []);

    const deleteMessage = (messageId) => {
        axios.delete(`http://localhost:8000/api/deleteContact/${messageId}`, {
            headers: {
        ...
        .then((response) => {
            ...
            const newMessages = messages.filter((message) => message.id !== messageId); // Remove the offer from the offers state
            setMessages(newMessages); // Update the offers state
            window.alert('Message deleted successfully');
        })
        .catch((error) => {
            console.error('Failed to delete message:', error.message);
            window.alert('Failed to delete message !', error.message);
        });
    };
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {messages.map((message) => (
                      <tr key={message.id}>
                        ...
                        <button>
                          <span className="ml-2 text-red-600 hover:text-red-900" onClick={() => deleteMessage(message.id)}>Delete</span>
                        </button>
                        </td>
                        </tr>
                    ))}
