...
const ResetPassword = () => {
    ...
    // Set the token once the component has mounted and the window object is available
    useEffect(() => {
        const urlParams = new URLSearchParams(window.location.search); // Get the query parameters from the URL
        const token = urlParams.get('token');
        if (token) {
            setToken(token); // Set the token from the query parameters
        }
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/password/reset', {
                email,  // Change the key to email
                password, // Change the key to password
                password_confirmation: passwordConfirmation, // Change the key to password_confirmation
                token, // Change the key to token
            });
